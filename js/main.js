...
$(function() {
	...
	function setColors(time, page) {
		...
		$('body').stop('colors', true,false).animate({
			'background-color': TAB_INFO[page].colors.bg,
			'color' : TAB_INFO[page].colors.fg
		}, {duration:time, queue: 'colors'} ).dequeue('colors');

		$('nav').stop('colors', true,false).animate({
			'background-color' : TAB_INFO[page].colors.nav,
			'border-color': TAB_INFO[page].colors.border
	...
	function semiload(page, skippush=false) {
		$.get(window.location.origin+'/inner/'+page, function(data) {
			var idx = data.indexOf('<title>');
			if(idx >= 0) {
				var end = data.indexOf('</title>', idx);
				typeText($('.title>.wrap'), data.substring(idx+7, end), 40);
				data = data.substring(0,idx) + data.substr(end+8);
			}
            ...
            var colorizor = function(vine) {
                vine.color.hue += new paper.Color(TAB_INFO[page].colors.menu).hue
                    - new paper.Color(TAB_INFO[window.page].colors.menu).hue;
                /*vine.color.saturation *= (0.05 + new paper.Color(TAB_INFO[window.page].colors.menu).saturation)
                    / (0.05 + new paper.Color(TAB_INFO[page].colors.menu).saturation)
                vine.color.lightness *= (0.05 + new paper.Color(TAB_INFO[window.page].colors.menu).lightness)
                    / (0.05 + new paper.Color(TAB_INFO[page].colors.menu).lightness)*/
            }
	...
	$("nav li>a").each(function(index, value) {
        ...
        if(TAB_INFO[name]) {
            var menucolor = TAB_INFO[name].colors.menu;
    		cssstr += '#'+id+'> a { color :  '+menucolor +'; }\n'
//                + '#'+id+'> a:hover { text-shadow : 2px 2px 5px rgba('+ hexToRgb(menucolor) +', 0.5); }\n'
    		 	+ '#'+id+'.active > a { background-color :  '+TAB_INFO[name].colors.shadow +' !important; }\n';
